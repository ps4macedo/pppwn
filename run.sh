...
if [ $VMUSB = true ] ; then
 sudo rmmod g_mass_storage
  ...
  echo -e "\033[92mDisco USB:\033[93m Habilitado\033[0m" | sudo tee /dev/tty1
fi
if [ $PPPOECONN = true ] ; then
   echo -e "\033[92mAcesso à Internet:\033[93m Habilitado\033[0m" | sudo tee /dev/tty1
else   
   echo -e "\033[92mAcesso à Internet:\033[93m Desabilitado\033[0m" | sudo tee /dev/tty1
fi
...
if [[ ! $(ethtool $INTERFACE) == *"Link detected: yes"* ]]; then
   echo -e "\033[31mAguardando conexão\033[0m" | sudo tee /dev/tty1
   while [[ ! $(ethtool $INTERFACE) == *"Link detected: yes"* ]]
   do
      coproc read -t 2 && wait "$!" || true
   done
   echo -e "\033[32mConexão encontrada\033[0m\n" | sudo tee /dev/tty1
fi
...
if [[ $GHT == *"open"* ]] ; then
echo -e "\n\033[95mGoldhen encontrado, abortando pppwn\033[0m\n" | sudo tee /dev/tty1
if [[ $LEDACT == "status" ]] ;then
	echo none | sudo tee /sys/class/leds/PWR/trigger >/dev/null
...
echo -e "\n\033[37m\033[44m\033[1mPronto para desbloquear o console. \033[93mVamos para a tentativa 1...\033[0m" | sudo tee /dev/tty1
...
do
 ...
 if [[ $stdo  == "[+] Done!" ]] ; then
	echo -e "\033[32mDESBLOQUEADO!!!\033[0m\n" | sudo tee /dev/tty1
	...
	echo -e -n "\033[32m$stage_number - \033[0m" | sudo tee /dev/tty1
 
 elif [[ $stdo  == *"Scanning for corrupted object...failed"* ]] ; then

 	echo -e "\033[31mFALHA! \033[93m\n\nVamos para a tentativa $attempt...\033[0m" | sudo tee /dev/tty1
	((attempt++))
